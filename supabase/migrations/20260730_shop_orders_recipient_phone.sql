-- ─────────────────────────────────────────────
-- QUEST — WhatsApp de quien recibe el pedido
-- ─────────────────────────────────────────────
-- El comprador no siempre es el que retira: se compra para un hermano, se
-- manda de regalo, o lo busca alguien de la casa. Guardar a quién avisarle
-- por WhatsApp, aparte del que pagó, es lo que permite que el aviso llegue
-- a la persona correcta.
--
-- Es opcional: si viene vacío, el equipo usa buyer_phone. No se le agrega
-- una traba más al pago.
--
-- Requiere 20260729_shop_orders_buyer_phone.sql.
...
DROP FUNCTION IF EXISTS public.place_paid_order(uuid, integer, text, numeric, text, uuid, text, text, text, text, text);
...
BEGIN
  ...
  INSERT INTO public.shop_orders (
    ...
    user_id, buyer_name, buyer_email, buyer_phone, recipient_phone,
    paypal_order_id, status
  ) VALUES (
    v_code, v_prod.id, v_prod.name, p_qty, round(p_total / p_qty, 2), p_total, p_branch,
    p_user_id, p_buyer_name, p_buyer_email, p_buyer_phone, p_recipient_phone,
    p_paypal_order_id, v_status
  )
...
REVOKE EXECUTE ON FUNCTION public.place_paid_order(uuid, integer, text, numeric, text, uuid, text, text, text, text, text) FROM PUBLIC, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.place_paid_order(uuid, integer, text, numeric, text, uuid, text, text, text, text, text) TO service_role;
